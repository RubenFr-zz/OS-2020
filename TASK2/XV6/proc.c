...
//// TASK 2.1 - Schedule priority based
void
scheduler(void) {
    struct proc *p;
    struct cpu *c = mycpu();
    ...
    for (;;) {
        ...
        for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
            ...
            //// Get the RUNNABLE process with the highest priority (lowest -> highest)
            if (p->state != RUNNABLE)
                continue;

            highestP = p;
            for (tempP = ptable.proc; tempP < &ptable.proc[NPROC]; tempP++) {
                if ((tempP->state == RUNNABLE) && (highestP->priority > tempP->priority))
                    highestP = tempP;
            }
            if (highestP != 0)
                p = highestP;

//            cprintf("current highest priority = %d\n", p->priority);
//            cprintf("%s (pid %d) running - priority = %d\n", p->name, p->pid, highest_priority);
            //// END EDITING
